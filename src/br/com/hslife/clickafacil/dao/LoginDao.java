...
public class LoginDao {
    ...
    public void salvar(Login g) throws Exception {
    ...
    public void alterar(Login g) throws Exception {
        sessao = HibernateUtil.getInstance();
        transacao = sessao.beginTransaction();
        sessao.update(g);
        transacao.commit();
    ...
    public void excluir(Login g) throws Exception {
        sessao = HibernateUtil.getInstance();
        transacao = sessao.beginTransaction();
        sessao.delete(g);
        transacao.commit();
    ...
    public List listarTodos() throws Exception {
        sessao = HibernateUtil.getInstance();
        return sessao.createCriteria(Login.class).list();
    }

    public Login buscarPorLogin(String login) throws Exception {
        String hql = "from Login where usuarioLogin = :login";
        Query q = HibernateUtil.getInstance().createQuery(hql);
        q.setString("login", login);
        return (Login) q.list().get(0);
    }

    public Login buscarPorLogin(Integer id) throws Exception {
